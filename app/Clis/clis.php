...
$rollbackCommand->setDescription('Rollback a Dumper class from database/dumper/')
    ...
    ->setCode(function (InputInterface $input, OutputInterface $output) {
        ...
        if (!class_exists($class)) {
            $output->writeln("<error>❌ Class [$class] not found after requiring file.</error>");
            return Command::FAILURE;
        }
        ...
        if (!($instance instanceof \App\Core\Dumper)) {
            $output->writeln("<error>❌ [$class] must extend App\\Core\\Dumper.</error>");
            return Command::FAILURE;
        }

        if (!method_exists($instance, 'rollback')) {
            $output->writeln("<error>❌ Method rollback() not found in [$class].</error>");
            return Command::FAILURE;
        }

        $output->writeln("<info>⏪ Rolling back:</info> $class");
